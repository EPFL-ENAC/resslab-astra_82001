<template>
  <section v-if="!rBau && bridgeType" class="longitudinal-verification row2">
    <section class="longitudinal-content">
      <section
        class="longitudinal-inputs"
        aria-labelledby="longitudinal-inputs"
      >
        <h3 class="longitudinal-header">
          {{ $t('longitudinal_verification') }}
        </h3>
      ...
      <section class="longitudinal-image" aria-label="longitudinal-image">
        <h3
          v-if="bridgeType === 'Slab' || bridgeType === 'Multi'"
          class="longitudinal-image-header"
        >
          {{ $t('trans') }}
          <!-- cf https://github.com/EPFL-ENAC/resslab-astra_82001/issues/43 -->
          <q-icon
            v-if="bridgeType === 'Multi'"
            class="cursor-pointer"
            :name="mdiInformationVariantCircleOutline"
          >
            <q-tooltip class="text-body1" html>
              <span v-html="$t('i12_desc')"></span
            ></q-tooltip>
          </q-icon>
        </h3>
        <q-img
          v-if="bridgeType === 'Box'"
          src="/box-longitudinal.svg"
          alt="Longitudinal Verification"
          style="height: 150px"
          fit="contain"
        />
        <!-- this is the image -->
        <q-img
          ...
          src="/twin-girder-longitudinal-composite.svg"
          alt="Twin girder composite"
          style="height: 150px"
          fit="contain"
        />
        <q-img
          v-else-if="bridgeType === 'Twin' && bridgeComposition === 'Concrete'"
          src="/twin-girder-longitudinal-concrete.svg"
          alt="twin girder concrete"
          style="height: 150px"
          fit="contain"
        />
        <p v-else-if="bridgeType === 'Multi'">
          <q-img
            src="/multi-girder-longitudinal-1.svg"
            alt="Multi girder "
            style="height: 125px"
            fit="contain"
          />
          <q-img
            src="/multi-girder-longitudinal-2.svg"
            alt="Multi girder "
            style="height: 125px"
            fit="contain"
          />
        ...
        <q-img
          v-else-if="bridgeType === 'Slab'"
          src="/slab-longitudinal.svg"
          alt="Slab"
          fit="contain"
          style="height: 150px"
        />
        ...
        <section
          v-if="bridgeType === 'Multi'"
          class="longitudinal-inputs longitudinal-radio q-mt-sm"
          aria-label="beams"
        >
  ...
  <section
    ...
    class="longitudinal-verification-result longitudinal-results alpha-footer"
    aria-lable=""
  >
    ...
    >
      <q-tooltip class="text-body1" html>
        <span v-html="$t('i9_desc')"></span
      ></q-tooltip>
    </q-icon>
    <ul class="alpha-list">
      <li class="alpha-item">
        &alpha;<sub>q,V</sub> &equals;
        {{ roundCeilWith2Decimals(alpha?.V?.[selectedClass]) }}
      ...
      <li class="alpha-item">
        ...
        {{ roundCeilWith2Decimals(alpha?.Mn?.[selectedClass]) }}
      ...
      <li class="alpha-item">
        &alpha;<sub>q,M+</sub> &equals;
        {{ roundCeilWith2Decimals(alpha?.Mp?.[selectedClass]) }}
...
const isConcrete = computed({
  ...
  set: (value) =>
    verificationStore.setBridgeComposition(value ? 'Concrete' : 'Composite'),
});
const selectedClass: ComputedRef<classResult> = computed(() =>
